...
function City() {
  const listCity = ["Buenos Aires", "Cordoba", "Cali", "Santiago", "Damasco","Tokio", "Japon", "Paris", "Roma", "Nueva York", "Barcelona", "California", "Amsterdam", "Manchester","Madrid", "Berlin", "Moscu", "Lyon"]
...
function CardElement(data) {
  ...
  const cardContainer = `
    ...
    <img src="${data.list[0].sys.pod === "n" ? "./img/background/dark.jpg" : "./img/background/light.jpg"}" alt="rover" />
</div>
...
<div class="card-body">
    <span class="tag ${data.list[0].sys.pod === "n" ? "tag-dark" : "tag-day"}">${data.city.name}</span>
